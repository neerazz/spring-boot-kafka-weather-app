...
~~~shell
minikube image load neerazz/spring-kafka-city:latest
~~~
...
your [JavaSample application](https://github.com/neerazz/spring-kafka-city/), and in the end,
...
- The first Kubernetes resource is
  ...
  - A Deployment creates and runs
    your [JavaSample application](https://github.com/neerazz/spring-kafka-city/tree/spring_boot_with_docker_k8) containers and
    keeps them alive.
...
- Next resource that you need
  is  [Service](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.20/#service-v1-core).
    - To expose your [JavaSample application](https://github.com/neerazz/spring-kafka-city/tree/spring_boot_with_docker_k8)
      you
      need Service.
...
- #### You can now access your application through the `spring-kafka-city` Service. In Minikube, a Service can be accessed with the following command:

```shell
minikube service spring-kafka-city --url
```

- This command should print the URL of `spring-kafka-city` Service.
...
```shell
kubectl scale --replicas=2 deployment/spring-kafka-city
```

#### You can watch how a new Pod is created with:

```shell
kubectl get pods -l app=spring-kafka-city --watch
```

> The `-l` flag restricts the output to only those Pods with an app=spring-kafka-city label.
